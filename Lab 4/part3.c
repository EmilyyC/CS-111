...
void printTemperature() {
	...
	while(1) {
		...
		if (!isStopped) {
			// write to server 
			if (SSL_write(SSLStruct, temperatureOutput, strlen(temperatureOutput)+1) < 0) {
...
int main() {
	...
	// SET UP THE TLS FUNCTIONALITY
	// load encryption and hash algorithms for SSL
	SSL_library_init();
	...
	// set the file descriptor sockfd as the input/output facility for the TLS/SSL (encrypted) side of ssl
	if (SSL_set_fd(SSLStruct, sockfd) == 0) {
		error("ERROR with setting file descriptor as SSL");
	}
	// initiate the TLS/SSL handshake with an TLS/SSL server
	if (SSL_connect(SSLStruct) != 1) {
		error("ERROR with TLS/SSL handshake");
	}

	// ACTUAL CODE BEGINS HERE
    // open the output file
    outputFile = fopen("lab4_3.log", "w");

  	// write my UID as the device identifier
  	if (SSL_write(SSLStruct, UID, strlen(UID)+1) < 0) {
  		error("ERROR writing device ID to server");
  	}
  	...
  	while (1) {
  		...
  		if (SSL_read(SSLStruct, commandBuffer, 1024) > 0) {
  	...
  	// close the socket
  	close(sockfd);
  	// shut down an active TLS/SSL connection
